...
# === MODIFICATION START: Import Additions ===
from typing import Optional, Union, Tuple
import time

# Toggle this flag to True to enable layer-by-layer extraction timing
TRACK_TIME = False
# === MODIFICATION END ===
...
def eager_attention_forward(
    ...
    **kwargs: Unpack[TransformersKwargs],
    ...
    if custom_mask:
        ...
        # attn_weights = attn_weights * reverse_custom_mask.to(attn_weights.device).to(attn_weights.dtype)
    # === MODIFICATION END ===
...
# @use_kernelized_func(apply_rotary_pos_emb) # remove because it cannot be imported
# === MODIFICATION END ===
    ...
    # === MODIFICATION END ===
        ...
        if extract_intermediate:
            if unit_test:
                ...
                test_reconstruction(attn_output.cpu(), reconstructed_output.cpu(), '[MODELING_OLMO2][DECOMPOSE][EAGER_ATT]', 'strict' if attn_output.dtype == torch.float32 else 'loose')
        ...
        if extract_intermediate and unit_test:
            ...
            test_reconstruction(attn_output.cpu(), reconstructed_output.cpu(), '[MODELING_OLMO2][DECOMPOSE][OUT_ATT]', atol=1e-4 if attn_output.dtype == torch.float32 else 0.15)
...
class Olmo2DecoderLayer(GradientCheckpointingLayer):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if extract_intermediate and TRACK_TIME:
            intermediate_representations['time'] = {}
        ...
        if extract_intermediate:
            if TRACK_TIME:
                if hidden_states.is_cuda: torch.cuda.synchronize()
                t0_before_attn = time.time()
                
            intermediate_representations['before_attn'] = hidden_states.detach().cpu().clone()
            
            if TRACK_TIME:
                if hidden_states.is_cuda: torch.cuda.synchronize()
                intermediate_representations['time']['before_attn'] = time.time() - t0_before_attn

        """ HEAD_OUT and ATTN_OUT"""
        # Start timer BEFORE the attention forward pass
        if extract_intermediate and TRACK_TIME:
            if hidden_states.is_cuda: torch.cuda.synchronize()
            t0_attn = time.time()
        ...
        if extract_intermediate:
            ...
            if TRACK_TIME:
                if decomposed_attn is not None and decomposed_attn.is_cuda: torch.cuda.synchronize()
                intermediate_representations['time']['head_out'] = time.time() - t0_attn

        # Start timer BEFORE the post-attention norm
        if extract_intermediate and TRACK_TIME:
            if hidden_states.is_cuda: torch.cuda.synchronize()
            t0_attn_out = time.time()
        ...
        if unit_test:
            ...
            test_reconstruction(hidden_states.cpu(), reconstructed_output, '[MODELING_OLMO2][DECOMPOSE][NORM_ATT]', 'strict' if hidden_states.dtype == torch.float32 else 'loose')

        if extract_intermediate:
            intermediate_representations['attn_out'] = hidden_states.detach().cpu().clone()
            if TRACK_TIME:
                if hidden_states.is_cuda: torch.cuda.synchronize()
                intermediate_representations['time']['attn_out'] = time.time() - t0_attn_out
        ...
        if extract_intermediate:
            if TRACK_TIME:
                if hidden_states.is_cuda: torch.cuda.synchronize()
                t0_before_mlp = time.time()
                
            intermediate_representations['before_mlp'] = hidden_states.detach().cpu().clone()
            
            if TRACK_TIME:
                if hidden_states.is_cuda: torch.cuda.synchronize()
                intermediate_representations['time']['before_mlp'] = time.time() - t0_before_mlp

        # Fully Connected
        residual = hidden_states
        
        # Start timer BEFORE the MLP forward pass
        if extract_intermediate and TRACK_TIME:
            if hidden_states.is_cuda: torch.cuda.synchronize()
            t0_mlp = time.time()
        ...
        if extract_intermediate:
            ...
            if TRACK_TIME:
                if hidden_states.is_cuda: torch.cuda.synchronize()
                intermediate_representations['time']['mlp_out'] = time.time() - t0_mlp
        ...
        if extract_intermediate:
            if TRACK_TIME:
                if hidden_states.is_cuda: torch.cuda.synchronize()
                t0_after_mlp = time.time()
                
            intermediate_representations['after_mlp'] = hidden_states.detach().cpu().clone()
            
            if TRACK_TIME:
                if hidden_states.is_cuda: torch.cuda.synchronize()
                intermediate_representations['time']['after_mlp'] = time.time() - t0_after_mlp
...
class Olmo2Model(Olmo2PreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        # === MODIFICATION START: Manual Causal Mask Fallback (Graph Compilation Opt-out) ===
        # DEPRECATED: This might not be useful with attn_implementation=eager
        # If Hugging Face optimized away the mask because there's no padding,
        # we MUST manually create the lower-triangular causal mask for our graph.
        if causal_mask is None:
            seq_len = inputs_embeds.shape[1]
            
            # 1. Create a matrix filled with the minimum possible float value (-inf equivalent)
            ...
            # 2. Use torch.triu to keep the upper triangle as -inf, 
            # and fill the lower triangle with 0.0 (meaning "do not mask")
        ...
        for i, decoder_layer in enumerate(self.layers[: self.config.num_hidden_layers]):
            ...
            if build_graph:
                ...
                if intermediate_representations:
                    # 1. Print Timing for the layer that just finished
                    if TRACK_TIME:
                        times = intermediate_representations.get('time', {})
                        if times:
                            total_time = sum(times.values())
                            timing_str = " | ".join([f"{k}: {v:.4f}s" for k, v in times.items()])
                            print(f"[LLM_STRACE] Layer {i-1} Extr. Time: {total_time:.4f}s [{timing_str}]")
            ...
            """ Move this layer to the GPU """

            # # === MODIFICATION START: Move Layer to GPU ===
            # decoder_layer = decoder_layer.to(hidden_states.device)
            # # === MODIFICATION END ===
            ...
            # # === MODIFICATION START: Move Layer back to CPU ===
        ...
        # Start timer BEFORE the final norm
        if TRACK_TIME and build_graph:
            if hidden_states.is_cuda: torch.cuda.synchronize()
            t0_final = time.time()
        ...
        if build_graph:
            ...
            if TRACK_TIME:
                if hidden_states.is_cuda: torch.cuda.synchronize()
                intermediate_representations['time']['final_norm'] = time.time() - t0_final
                
                # Print Timing for the final layer
                times = intermediate_representations.get('time', {})
                if times:
                    total_time = sum(times.values())
                    timing_str = " | ".join([f"{k}: {v:.4f}s" for k, v in times.items()])
                    print(f"[LLM_STRACE] Layer {i} Extr. Time: {total_time:.4f}s [{timing_str}]")
            
            if unit_test:
                test_reconstruction(
                    intermediate_representations['after_mlp'],
                    intermediate_representations['before_mlp'] + intermediate_representations['mlp_out'],
                    '[MODELING_OLMO2][DECODER_LAYER][FINAL NORM]', 'strict' if hidden_states.dtype == torch.float32 else 'loose')
                test_reconstruction(
                    hidden_states.cpu(),
                    intermediate_representations['after_mlp'],
                    '[MODELING_OLMO2][DECODER_LAYER][FINAL NORM 2]', 'strict')
            for rep in ['before_mlp', 'mlp_out', 'after_mlp']:
