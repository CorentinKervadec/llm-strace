    ...
    # === MODIFICATION END ===
        ...
        if extract_intermediate:
            if unit_test:
                ...
                test_reconstruction(attn_output.cpu(), reconstructed_output, '[MODELING_QWEN2][DECOMPOSE][EAGER_ATT]', 'strict' if attn_output.dtype == torch.float32 else 'loose')
        ...
        if extract_intermediate and unit_test:
            ...
            test_reconstruction(attn_output.cpu(), reconstructed_output.cpu(), '[MODELING_QWEN2][DECOMPOSE][OUT_ATT]', atol=1e-4 if attn_output.dtype == torch.float32 else 0.15)
        ...
        if build_graph:
            ...
            if unit_test:
                test_reconstruction(
                    intermediate_representations['after_mlp'],
                    intermediate_representations['before_mlp'] + intermediate_representations['mlp_out'],
                    '[MODELING_QWEN2][DECODER_LAYER][FINAL NORM]', 'strict' if hidden_states.dtype == torch.float32 else 'loose')
                test_reconstruction(
                    hidden_states.cpu(),
                    intermediate_representations['after_mlp'],
                    '[MODELING_QWEN2][DECODER_LAYER][FINAL NORM 2]', 'strict')
            for rep in ['before_mlp', 'mlp_out', 'after_mlp']:
