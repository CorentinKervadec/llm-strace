...
def eager_attention_forward(
    ...
    **kwargs: Unpack[TransformersKwargs],
    ...
    if unit_test and attn_weights.dtype==torch.float32:
        ...
        test_reconstruction(attn_weights_original, attn_weights, '[MODELING_LLAMA][DECOMPOSE][ATT_WEIGTH]', 'strict')
    # === MODIFICATION END ===
    ...
    # === MODIFICATION END ===
        ...
        if extract_intermediate:
            if unit_test:
                ...
                test_reconstruction(attn_output.cpu(), reconstructed_output.cpu(), '[MODELING_MISTRAL][DECOMPOSE][EAGER_ATT]', 'strict' if attn_output.dtype == torch.float32 else 'loose')
...
class MistralRotaryEmbedding(nn.Module):
    ...
    def __init__(self, config: MistralConfig, device=None):
        ...
        # Modified because 'MistralConfig' object has no attribute 'rope_parameters'
...
class MistralModel(MistralPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        # === MODIFICATION START: Disabling Sliding Window ===
        ...
        if build_graph:
            ...
            if unit_test:
                test_reconstruction(
                    intermediate_representations['after_mlp'],
                    intermediate_representations['before_mlp'] + intermediate_representations['mlp_out'],
                    '[MODELING_MISTRAL][DECODER_LAYER][FINAL NORM]', 'strict' if hidden_states.dtype == torch.float32 else 'loose')
                test_reconstruction(
                    hidden_states.cpu(),
                    intermediate_representations['after_mlp'],
                    '[MODELING_MISTRAL][DECODER_LAYER][FINAL NORM 2]', 'strict')
            for rep in ['before_mlp', 'mlp_out', 'after_mlp']:
