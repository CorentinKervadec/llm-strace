...
def plot_multi_model_results(all_models_data: dict, output_pdf: str):
    ...
    with PdfPages(output_pdf) as pdf:
        ...
        for i, (model_name, data) in enumerate(all_models_data.items()):
            ...
            # Changed to errorbar with fmt='o' (points)
            ax.errorbar(x_data, y_mean, yerr=y_std, label=model_name, color=colors(i), 
                        fmt='o', markersize=4, capsize=3, alpha=0.8)
...
def main():
    ...
    for method_name in sorted(model_dirs):
        ...
        print(f"\n--- Loading data for model: {method_name} ---")
        ...
        if static_data and aggregated_data:
            ...
            print(f"Successfully loaded and aggregated data for {method_name}.")
        else:
            print(f"Warning: No valid data loaded for {method_name}. Skipping this model.")
